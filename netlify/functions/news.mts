...
export default async function handler(req: Request) {
  ...
  if (req.method === 'DELETE') {
    const result = await db.sql`DELETE FROM news_articles WHERE id = ${id} RETURNING id`;
    return result.length ? json({ deleted: id }) : json({ error: 'News article not found.' }, { status: 404 });
  }
  ...
  if (req.method === 'PUT') {
    ...
    return updated ? json(updated) : json({ error: 'News article not found.' }, { status: 404 });
  }
