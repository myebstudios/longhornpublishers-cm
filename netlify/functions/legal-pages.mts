...
export default async function handler(req: Request) {
  ...
  const [saved] = await db.sql`
    INSERT INTO legal_pages (page, body_en, body_fr) VALUES (${value.page}, ${value.body_en}, ${value.body_fr})
    ON CONFLICT (page) DO UPDATE SET body_en = EXCLUDED.body_en, body_fr = EXCLUDED.body_fr, updated_at = now()
    RETURNING *
  `;
  await requestRebuild(`${value.page} updated`);
  return json(saved);
}
