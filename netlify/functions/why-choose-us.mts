...
export default async function handler(req: Request) {
  ...
  const [saved] = await db.sql`
    INSERT INTO why_choose_us (id, local_presence_copy_en, local_presence_copy_fr, quality_commitment_items)
    VALUES ('default', ${value.local_presence_copy_en}, ${value.local_presence_copy_fr}, ${JSON.stringify(value.quality_commitment_items)})
    ...
    RETURNING *
  `;
  await requestRebuild('why choose us page updated');
  return json(saved);
}
