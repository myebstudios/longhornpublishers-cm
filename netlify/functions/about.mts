...
export default async function handler(req: Request) {
  ...
  const [saved] = await db.sql`
    INSERT INTO about_page (
      ...
      mission_en, mission_fr, values_en, values_fr, team_capacity_blocks
    ) VALUES (
      ...
      ${value.values_en}, ${value.values_fr}, ${JSON.stringify(value.team_capacity_blocks)}
      ...
      team_capacity_blocks = EXCLUDED.team_capacity_blocks, updated_at = now()
    RETURNING *
  `;
  await requestRebuild('about page updated');
  return json(saved);
}
