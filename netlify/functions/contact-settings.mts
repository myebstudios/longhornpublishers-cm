...
export default async function handler(req: Request) {
  ...
  const [saved] = await db.sql`
    INSERT INTO contact_settings (id, hero_copy_en, hero_copy_fr, project_type_options, map_lat, map_lng)
    VALUES ('default', ${value.hero_copy_en}, ${value.hero_copy_fr}, ${JSON.stringify(value.project_type_options)}, ${value.map_lat}, ${value.map_lng})
    ON CONFLICT (id) DO UPDATE SET hero_copy_en = EXCLUDED.hero_copy_en, hero_copy_fr = EXCLUDED.hero_copy_fr,
      project_type_options = EXCLUDED.project_type_options, map_lat = EXCLUDED.map_lat, map_lng = EXCLUDED.map_lng, updated_at = now()
    RETURNING *
  `;
  await requestRebuild('contact settings updated');
  return json(saved);
}
